// Generated from ./antlr/json/generated/JSON.g4 by ANTLR 4.13.1
// jshint ignore: start
import antlr4 from 'antlr4';

// This class defines a complete listener for a parse tree produced by JSONParser.
export default class JSONListener extends antlr4.tree.ParseTreeListener {

	// Enter a parse tree produced by JSONParser#json.
	enterJson(ctx) {
	}

	// Exit a parse tree produced by JSONParser#json.
	exitJson(ctx) {
	}


	// Enter a parse tree produced by JSONParser#obj.
	enterObj(ctx) {
	}

	// Exit a parse tree produced by JSONParser#obj.
	exitObj(ctx) {
	}


	// Enter a parse tree produced by JSONParser#pair.
	enterPair(ctx) {
	}

	// Exit a parse tree produced by JSONParser#pair.
	exitPair(ctx) {
	}


	// Enter a parse tree produced by JSONParser#key.
	enterKey(ctx) {
	}

	// Exit a parse tree produced by JSONParser#key.
	exitKey(ctx) {
	}


	// Enter a parse tree produced by JSONParser#arr.
	enterArr(ctx) {
	}

	// Exit a parse tree produced by JSONParser#arr.
	exitArr(ctx) {
	}


	// Enter a parse tree produced by JSONParser#String.
	enterString(ctx) {
	}

	// Exit a parse tree produced by JSONParser#String.
	exitString(ctx) {
	}


	// Enter a parse tree produced by JSONParser#Number.
	enterNumber(ctx) {
	}

	// Exit a parse tree produced by JSONParser#Number.
	exitNumber(ctx) {
	}


	// Enter a parse tree produced by JSONParser#Object.
	enterObject(ctx) {
	}

	// Exit a parse tree produced by JSONParser#Object.
	exitObject(ctx) {
	}


	// Enter a parse tree produced by JSONParser#Array.
	enterArray(ctx) {
	}

	// Exit a parse tree produced by JSONParser#Array.
	exitArray(ctx) {
	}


	// Enter a parse tree produced by JSONParser#Boolean.
	enterBoolean(ctx) {
	}

	// Exit a parse tree produced by JSONParser#Boolean.
	exitBoolean(ctx) {
	}


	// Enter a parse tree produced by JSONParser#Null.
	enterNull(ctx) {
	}

	// Exit a parse tree produced by JSONParser#Null.
	exitNull(ctx) {
	}



}